...
Go service serving a live CAN console on the WiFi AP at **`http://192.168.244.1`**
...
- **Browser note:** use **Safari or Firefox**. Chrome with "Always use secure
  connections" upgrades `http://192.168.244.1` to HTTPS (port 443, not served) and
  ...
  connections". `curl http://192.168.244.1/` on the car always works.
